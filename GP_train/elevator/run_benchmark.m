% RUN_BENCHMARK  Elevators kernel-ridge PCG benchmark over a regularization sweep.
%
% Builds the dense RBF kernel K once on an n-point Elevators subset and, for each
...
params.n          = 2000;                 % kernel subset size
...
rng(params.Seed);
[X, y]  = load_dataset_csv_or_mat(fullfile(thisFileDir, 'data', 'elevators.csv'));
...
fprintf('[elevator] n=%d  ell0=%.4f  ell=%.4f  building dense K ...\n', params.n, ell0, ell);
...
title(sprintf('Elevators kernel PCG: iterations vs regularization (n=%d, %d largest eigvecs)', ...
...
cfg_out.data_file     = fullfile(thisFileDir, 'data', 'elevators.csv');
cfg_out.ell0          = ell0;
cfg_out.ell           = ell;
...
fprintf('\n[elevator run_benchmark] done. Output in %s\n', outDir);
