...
This directory now contains two complementary experiments:

- `run_benchmark.m`: the original fixed-kernel diagonal-shift sweep described
  below.
- `run_ard_training_benchmark.m`: an end-to-end GP hyperparameter-training
  benchmark in which four ARD lengthscales, signal variance, and noise variance
  change the dense system throughout optimization.

## ARD training benchmark

The ARD experiment solves a sequence

```text
A(theta) = signal_variance * K_ARD(ell_1,...,ell_4) + noise_variance * I
```

generated by marginal-likelihood optimization. Unlike a regularization-only
sweep, changes to the four feature lengthscales change the off-diagonal kernel
entries and rotate its eigenspaces. Each optimizer state requires one solve for
the target and eight fixed Hutchinson-probe solves for the stochastic trace in
...
Run the fast end-to-end configuration with:
...
After training, the benchmark selects the initial state, the state with maximum
...
targeted without changing the primary training methods prematurely.

ARD results are written to `benchmark_ard_training/` (or
`benchmark_ard_training_smoke/`):
...
Exact final likelihood, held-out RMSE, and spectrum calculations are diagnostic
and excluded from timed preconditioner/solve totals.

The workload follows the standard GP marginal-likelihood formulation in
Rasmussen and Williams, and the iterative solve/trace-estimation setting used
in published GP preconditioning studies:

- Rasmussen & Williams, *Gaussian Processes for Machine Learning*, ch. 2:
  https://gaussianprocess.org/gpml/chapters/RW2.pdf
- Cutajar et al., *Preconditioning Kernel Matrices*:
  https://proceedings.mlr.press/v48/cutajar16.html
- Wenger et al., *Preconditioning for Scalable Gaussian Process
  Hyperparameter Optimization*:
  https://proceedings.mlr.press/v162/wenger22a.html
