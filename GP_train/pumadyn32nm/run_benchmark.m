% RUN_BENCHMARK  pumadyn32nm kernel-ridge PCG benchmark over a regularization sweep.
%
% Builds the dense RBF kernel K once on an n-point pumadyn32nm subset and, for each
...
rng(params.Seed);
[X, y]  = load_dataset_csv_or_mat(fullfile(thisFileDir, 'data', 'pumadyn32nm.csv'));
...
fprintf('[pumadyn32nm] n=%d  ell0=%.4f  ell=%.4f  building dense K ...\n', params.n, ell0, ell);
...
title(sprintf('pumadyn32nm kernel PCG: iterations vs regularization (n=%d, %d largest eigvecs)', ...
...
cfg_out.data_file     = fullfile(thisFileDir, 'data', 'pumadyn32nm.csv');
cfg_out.ell0          = ell0;
cfg_out.ell           = ell;
...
fprintf('\n[pumadyn32nm run_benchmark] done. Output in %s\n', outDir);
