...
% --- T3  the cached block matches, and is NOT repaired -------------------
SelYSel_ref = full(ctx.Sel' * ctx.YSel);
relBlk = norm(ctx.SelYSel - SelYSel_ref, 'fro') / ...
         max(norm(ctx.SelYSel, 'fro'), eps);
[np, nf] = chk(np, nf, ...
    sprintf('T3a SelYSel == Sel''*YSel bit-for-bit (%.2e)', relBlk), ...
    isequal(ctx.SelYSel, SelYSel_ref));

% T3b USED to assert exact symmetry, which held only because the block was
% symmetrized on the way in.  That repair was removed: woodbury_solve is a naive
% evaluation and must not be handed pre-cleaned inputs, or the accuracy it reports
% would be the repair's and not the problem's.  The asymmetry is now measured and
% required to sit at rounding level -- a real check, since anything larger means
% YSel or Sel is wrong rather than that rounding occurred.
asym = norm(ctx.SelYSel - ctx.SelYSel', 'fro') / ...
       max(norm(ctx.SelYSel, 'fro'), eps);
[np, nf] = chk(np, nf, ...
    sprintf('T3b SelYSel asymmetry is at rounding level, unrepaired (%.2e < 1e-13)', ...
            asym), ...
    asym > 0 && asym < 1e-13);
