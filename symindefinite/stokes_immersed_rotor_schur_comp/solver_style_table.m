...
%   linestyle, because in this benchmark curves genuinely coincide: when the
%   sketched coarse space recovers the same modes as the exact one,
%   deflate_gaussian and deflate_exact sit on top of each other.
%
...
%
%   LOCAL COPY, extended to TWELVE colours.  The four arms this study now
%   registers fit inside the original eight, but the extra rows are kept so that
%   adding arms back (an sm_eig ablation, an sjlt sketch) cannot silently wrap
%   the palette and give two arms the same colour.  Marker and linestyle would
%   still differ, but colour is what the eye reads first.
%
%   Beyond twelve the table cycles: colours advance one step out of phase with
%   markers, so the repeat period is 12*8 rather than 12.
%
%   See also: write_schur_case_outputs, write_schur_summary.
