...
for k = 1:numel(STEPS)
    ...
    meta = struct( ...
        ...
        'step', n, 't_snap', tcur, ...
        ... % Read the angle from what the factory actually built.  The old
        ... % literal 2*pi*2*tcur/Tmax baked in BOTH nrev = 2 and theta0 = 0, so
        ... % it silently mis-reported the geometry once theta0 became a knob.
        'theta_rad', bar_angle(Sq, tcur), ...
        'theta_deg', rad2deg(bar_angle(Sq, tcur)), ...
...
%==========================================================================
function th = bar_angle(Sq, tcur)
%BAR_ANGLE  The rotating bar's angle at time TCUR, from the sequence's own metadata.
%   build_stokes_sequence records motion_meta (omega, theta0, nrev) as the factory
%   built it, so this cannot drift from define_motion_list the way a duplicated
%   literal did.  This runs with use_cache = false, and a validated cache hit
%   stamps motion_meta too, so the field is always present.
    assert(isfield(Sq, 'motion_meta') && strcmp(Sq.motion_meta.kind, 'bar_rotating'), ...
           ['theta is only defined for bar_rotating, and requires motion_meta ' ...
            '(rebuild the sequence if it came from a pre-motion_meta cache).']);
    th = Sq.motion_meta.theta0 + Sq.motion_meta.omega * tcur;
end
