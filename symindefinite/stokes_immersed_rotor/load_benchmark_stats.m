function [all_stats, cfg] = load_benchmark_stats(results_root)
...
%
%   See also: write_all_results_csv, replot_benchmark, write_case_figures.

    in_csv = fullfile(results_root, 'all_results.csv');
    if ~exist(in_csv, 'file')
        error('load_benchmark_stats:noCsv', ...
    ...
    if isempty(keys)
        error('load_benchmark_stats:noSolvers', ...
...
function cfg = read_run_config(results_root)
    ...
    if exist(jsonFile, 'file')
        try
            ...
            warning('load_benchmark_stats:badJson', ...
