function tags = varvisc_mark_coincident_curves(stats)
...
%
%   See also: varvisc_solver_short_label, varvisc_plot_solver_curves.
    ...
    for s = 2:n
        ...
        for r = 1:s-1
            ...
            if isequal(size(vs), size(vr)) && isequal(vs, vr)
                tags{s} = sprintf(' (= %s)', varvisc_solver_short_label(keys{r}));
                break
            end
        end
    end
