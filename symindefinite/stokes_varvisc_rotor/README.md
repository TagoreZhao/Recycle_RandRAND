...
| `two_level_lowrank_sketch` | ILDL + deflation, V = randomized sketch of $D = K_1^{-1}(K_n-K_1)$ (the frozen *factorization* reused, not a subspace) |
...
Every randomized sketch — gaussian V, sjlt V, and the low-rank $D$-sketch —
...
as large.  Sketch cost is reported in operation counts (the low-rank arm
pays $(2q{+}1)\cdot k$ batched backsolves against the frozen factor per
build), never wall-clock.
...
- Control case: frozen $\equiv$ refreshed (curves coincide), and the
  low-rank arm degrades to plain ILDL ($\Delta K = 0$ returns an empty V —
