function varvisc_write_case_figures(run_dir, stats, opts)
...
%       accuracy.png
%
%   STATS follows the contract in varvisc_load_benchmark_stats, so run_benchmark (live)
%   and replot_varvisc_benchmark (from CSV) produce byte-comparable figures.
%
%   See also: varvisc_plot_solver_curves, varvisc_place_solver_legend, save_varvisc_figure.
...
% the subtitle at 9 pt, where it fits instead of running off the top edge.
    for s = 1:numel(keys)
        ...
        ax  = axes(fh); %#ok<LAXES>
        sty = varvisc_solver_style_table(numel(keys));
        ...
        title(ax, sprintf('%s  |  %s', stats.case_name, varvisc_solver_short_label(keys{s})), ...
...
function write_comparison_figure(run_dir, stats, tax, opts)
    ...
    ax = nexttile(tl);
    [h, legLabels] = varvisc_plot_solver_curves(ax, tax, stats, 't', opts);
    ...
    save_varvisc_figure(fh, fullfile(run_dir, 'all_solvers_comparison.png'), opts);
end

%==========================================================================
function write_coupling_change_figure(run_dir, stats, tax, opts)
% Per-step operator change: the fluid-block change diffK (the full-rank update
% this benchmark exists to measure) overlaid with the coupling-border change.
    fh = new_figure(opts.single_width, opts.single_height);
    ax = axes(fh);
    hold(ax, 'on');
    h = plot(ax, tax, stats.coupling_change(:), '.-', 'LineWidth', 1.4, ...
             'Color', [0.00 0.45 0.70]);
    lab = {'||\DeltaC||_F / ||C||_F'};
    if isfield(stats, 'diffK') && any(isfinite(stats.diffK(:)))
        h(end+1) = plot(ax, tax, stats.diffK(:), '.-', 'LineWidth', 1.4, ...
                        'Color', [0.84 0.37 0.00]);
        lab{end+1} = '||\DeltaF||_F / ||F||_F  (fluid block)';
    end
    hold(ax, 'off');
    grid(ax, 'on');
    xlabel(ax, 't');
    ylabel(ax, 'relative change');
    title(ax, sprintf('%s  |  per-step matrix change', stats.case_name), ...
          'Interpreter', 'none', 'FontSize', opts.titlefontsize, ...
          'FontWeight', 'bold');
    legend(ax, h, lab, 'FontSize', opts.legendfontsize, ...
           'Location', 'best', 'Box', 'on', 'EdgeColor', [0.65 0.65 0.65]);
    save_varvisc_figure(fh, ...
...
%   subtitle rather than relabelling a residual as an error.
    keys = stats.solver_keys;
    lastShort = varvisc_solver_short_label(keys{end});
    ...
    save_varvisc_figure(fh, fullfile(run_dir, 'accuracy.png'), opts);
end
