function [V, info, Y] = varvisc_build_lowrank_sketch_V(ctx, K, P, opts)
%VARVISC_BUILD_LOWRANK_SKETCH_V  Coarse space from a randomized sketch of D = A_1^{-1}(A_2 - A_1).
%
%   [V, info, Y] = VARVISC_BUILD_LOWRANK_SKETCH_V(CTX, K, P, OPTS)
%
%   CTX is a varvisc_frozen_ldl_context for the REFERENCE system A_1 = K_ref; K is the
...
%
%   THE SPAN IS NOT KNOWN IN ADVANCE.  Unlike the constant-viscosity parent
%   benchmark (where K_n - K_ref is the coupling border only, rank <= 2*nC, so a
%   wide enough sketch captures the WHOLE update), here the moving viscosity
%   field changes every nonzero of the fluid block and dK is numerically
%   FULL-RANK: k columns are a genuine truncation of the update's range, with no
%   rank ceiling to clear.  Expect info.rank_drop ~ 0 and info.ncols ~ k; the
%   EFFECTIVE dimension info.ncols, not k, remains the number to quote.
%
%   SKETCH PARAMETERS ARE UNIFIED with the deflation arms: the registry passes
%   k = SKETCH_OVERSAMPLE * DEFLAT_SM_EIG and q = DEFLAT_Q, so this sketch and
%   the gaussian/sjlt deflation sketches are parameter-identical, and V keeps
%   all k columns (orthonormalized, truncated only at numerical rank).
%
...
%
%   See also: varvisc_frozen_ldl_context, varvisc_frozen_ldl_apply, transport_V, orth_trunc,
%             src.precond.two_level_split_solve.
    ...
    Dfun  = @(X) varvisc_frozen_ldl_apply(ctx, dK * X);          % D  = K_ref^{-1} dK
    Dtfun = @(X) dK' * varvisc_frozen_ldl_apply(ctx, X);         % D' = dK' K_ref^{-1}
                                                         % (K_ref symmetric)
    nbs = 0;                                             % backsolves
