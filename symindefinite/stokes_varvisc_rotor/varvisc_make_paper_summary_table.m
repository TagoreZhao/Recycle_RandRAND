function varvisc_make_paper_summary_table(results_root)
...
% are added to varvisc_define_solver_list.  Reports mean/std iterations per solver and,
...
%                 to <thisDir>/benchmark_varvisc.  run_varvisc_benchmark passes
%                 its own results_root so the summary tracks redirected dirs.
    here    = fileparts(mfilename('fullpath'));
    if nargin < 1 || isempty(results_root)
        results_root = fullfile(here, 'benchmark_varvisc');
    end
