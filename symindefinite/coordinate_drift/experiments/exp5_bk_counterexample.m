...
%Tests Prop 3.3 (pattern flip and Bunch-Kaufman flip) and Prop 3.4 (|D|^{1/2}).
%
...
%   See also: ildl_coordinate_map, src.precond.make_ildl_precond, gap.
    ...
    V = [V, vrec('exp5A', 'Prop 3.3 an eta=1e-12 entry is enough', ...
    ...
    V = [V, vrec('exp5B', 'Prop 3.3 ldl flips 2x2 -> 1x1 exactly at alpha', ...
    ...
    V = [V, vrec('exp5B', 'Prop 3.3 the closed-form branches match ldl', ...
    ...
    V = [V, vrec('exp5C', 'Prop 3.3 one extra entry reorders AMD', ...
    ...
    V = [V, vrec('exp5D', 'Prop 3.4 kappa_2(C) ~ |d_min|^{-1/2}', ...
