...
[Vsmall,smallInfo] = gaussian_subspace_basis(Sinv,n,k,2,2);
[Vlarge,largeInfo] = gaussian_subspace_basis(Sapply,n,k,2,2);
assert(size(Vsmall,2) == 2*k && size(Vlarge,2) == 2*k, ...
       'Gaussian sketches did not retain all 2k basis vectors.');
assert(smallInfo.sketchWidth == 2*k && largeInfo.sketchWidth == 2*k, ...
       'Gaussian sketch width is not exactly 2k.');
assert(norm(Vsmall'*Vsmall-eye(2*k),'fro') < 1e-10, ...
       'Small Gaussian sketch basis is not orthonormal.');
assert(norm(Vlarge'*Vlarge-eye(2*k),'fro') < 1e-10, ...
       'Large Gaussian sketch basis is not orthonormal.');
[Vfractional,fractionalInfo] = gaussian_subspace_basis(Sapply,n,k,0,1.5);
assert(size(Vfractional,2) == ceil(1.5*k) && ...
       fractionalInfo.sketchWidth == ceil(1.5*k), ...
       'A noninteger oversampling factor did not set the retained width.');
