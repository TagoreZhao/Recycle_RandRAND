...
% span(V_true) into the Krylov space, via the local subspace_capture_directed.
% At Krylov dimension j < ktar the ktar - j uncapturable directions count as
% sin(theta) = 1.
%
% Outputs (subspace_capture/output_krylov_capture/):
%   results.mat / results.csv
%   angle_capture_fraction_A.pdf  — fraction of directions with sin(theta)<1%
%                                   vs Krylov dim j, Z=A
%   eigspace_err_fro_A.pdf        — Frobenius eigenspace error vs j, Z=A
%   angle_capture_fraction_Tsym.pdf — same, Z = L^{-1} A L^{-T}
%   eigspace_err_fro_Tsym.pdf
...
for is = 1:numel(systems)
    ...
    % Capture sweep over Krylov dimension j.  Directed principal-angle
    % metrics are basis-invariant; at j < kt the kt - j uncapturable
    % directions get sin(theta) = 1 by construction, so the capture
    % fraction stays meaningful for every j.  Both inputs are already
    % orthonormal (eigs cache / Lanczos with full reorthogonalization), so
    % the metric skips its internal pivoted QR on every call.
    capOpts = struct('true_is_orth', true, 'comp_is_orth', true);
    for it = 1:numel(ktars)
        kt = ktars(it);
        Vk = Vtrue(:, 1:kt);
        for j = 1:Jdim
            cap = subspace_capture_directed(Vk, Q(:, 1:j), [], capOpts);
            info = struct( ...
                'Z',                 Zk, ...
                'ktar', kt, 'jdim', j, ...
                'angle_capture_frac_1pct', cap.n_angle_below_1pct / kt, ...
                'n_angle_below_1pct',      cap.n_angle_below_1pct, ...
                'eigspace_err_2',          cap.eigspace_err_2, ...
                'eigspace_err_fro',        cap.eigspace_err_fro);
            rows = [rows; info];                                      %#ok<AGROW>
        end
        fprintf(['  ktar=%3d : at J=%d angle_capture_frac=%.3f (count=%d), ', ...
                 'err_fro=%.3e, err_2=%.3e\n'], ...
                kt, J, rows(end).angle_capture_frac_1pct, ...
                rows(end).n_angle_below_1pct, ...
                rows(end).eigspace_err_fro, rows(end).eigspace_err_2);
    end
end
...
%WRITE_RESULTS_CSV  Flat CSV of the capture sweep (directed-angle metrics).
    fid = fopen(csvPath, 'w');
    fprintf(fid, ['Z,ktar,jdim,angle_capture_frac_1pct,n_angle_below_1pct,', ...
                  'eigspace_err_2,eigspace_err_fro\n']);
    for i = 1:numel(rows)
        r = rows(i);
        fprintf(fid, '%s,%d,%d,%g,%g,%g,%g\n', ...
                r.Z, r.ktar, r.jdim, ...
                r.angle_capture_frac_1pct, r.n_angle_below_1pct, ...
...
%   Field `tag` is the file stem for the per-panel PDFs.
    specs = struct( ...
        'metric', {'angle_capture_frac_1pct',      'eigspace_err_fro', ...
                   'angle_capture_frac_1pct',      'eigspace_err_fro'}, ...
        'Z',      {'A',                            'A', ...
                   'Tsym',                         'Tsym'}, ...
        'Zlabel', {'A',                            'A', ...
                   'L^{-1} A L^{-T}',              'L^{-1} A L^{-T}'}, ...
        'yscale', {'linear',                       'log', ...
                   'linear',                       'log'}, ...
        'ylabel', {'captured directions (sin\theta < 1%)',  'eigenspace error (Frobenius)', ...
                   'captured directions (sin\theta < 1%)',  'eigenspace error (Frobenius)'}, ...
        'titlebit', {'Krylov angle capture fraction', 'Krylov eigenspace error', ...
                     'Krylov angle capture fraction', 'Krylov eigenspace error'}, ...
        'legloc', {'northwest',                    'northeast', ...
                   'northwest',                    'northeast'}, ...
        'tag',    {'angle_capture_fraction_A',     'eigspace_err_fro_A', ...
                   'angle_capture_fraction_Tsym',  'eigspace_err_fro_Tsym'});
end
