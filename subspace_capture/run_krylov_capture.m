...
% sin(theta) = 1.  Old per-column residuals are kept as CSV diagnostics.
%
...
for is = 1:numel(systems)
    ...
    % fraction stays meaningful for every j.
    for it = 1:numel(ktars)
        kt = ktars(it);
        Vk = Vtrue(:, 1:kt);
        for j = 1:Jdim
            cap = subspace_capture_directed(Vk, Q(:, 1:j));
            info = struct( ...
                'Z',                 Zk, ...
                'ktar', kt, 'jdim', j, ...
