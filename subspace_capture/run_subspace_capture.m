...
%   Field `tag` is the file stem for the per-panel PDFs.  Both metrics are
%   basis-invariant directed principal-angle quantities.
    specs = struct( ...
        'metric',     {'eigspace_err_2', 'angle_capture_frac_1pct', 'eigspace_err_2',   'angle_capture_frac_1pct'}, ...
        'Z',          {'A',              'A',                       'Tsym',             'Tsym'}, ...
        'Zlabel',     {'A',              'A',                       'L^{-1} A L^{-T}',  'L^{-1} A L^{-T}'}, ...
        'yscale',     {'log',            'linear',                  'log',              'linear'}, ...
        'ylabel',     {'||(I-P)Q_{true}||_2', 'captured directions (sin\theta < 1%)', ...
                       '||(I-P)Q_{true}||_2', 'captured directions (sin\theta < 1%)'}, ...
        'titlebit',   {'eigenspace error',  'angle capture fraction', 'eigenspace error', 'angle capture fraction'}, ...
        'legend_loc', {'southwest',      'northwest',               'south',            'south'}, ...
        'tag',        {'eigspace_err2_A', 'angle_capture_fraction_A', ...
                       'eigspace_err2_Tsym', 'angle_capture_fraction_Tsym'});
end
...
%     eigspace_err2_A.pdf            — ||(I-P)Q_true||_2 vs degree, Z = A
%     angle_capture_fraction_A.pdf   — fraction of directions with
%                                      sin(theta)<1%, Z = A
%     eigspace_err2_Tsym.pdf         — same, Z = L^{-1} A L^{-T}
%     angle_capture_fraction_Tsym.pdf
...
%   Large canvas so the (up to 9) per-tile legends stay legible.
    ...
    specs = panel_specs();
    order = [find_spec(specs,'A','eigspace_err_2'),    find_spec(specs,'A','angle_capture_frac_1pct'), ...
             find_spec(specs,'Tsym','eigspace_err_2'), find_spec(specs,'Tsym','angle_capture_frac_1pct')];
