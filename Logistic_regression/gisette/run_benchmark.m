% RUN_BENCHMARK  Gisette logistic-regression Newton-PCG preconditioner benchmark.
%
% Runs an instrumented Newton loop for L2-penalized logistic regression on the
% gisette dataset (n=6000 samples, d=5000 features) and, at every Newton
% iteration, solves the identical Newton system
...
% trajectory is advanced only by an exact reference solve delta = H_k \ (-g_k)
% on the explicit sparse Hessian (a direct Cholesky factorization; for gisette
% n=6000 > m=5001 the data term is full rank, so the m-by-m direct solve is the
% sensible reference rather than duke's n-by-n Woodbury form), followed by the
% same descent-guard + Armijo line search as logreg_newton. The benchmarked
...
%
% Caveat: ichol/AMG need an explicit H; standardized gisette X is dense so the
% sparse H_k is near-dense (~m^2 nnz, m = d+1 = 5001). nnz is printed once per
...
addpath(thisFileDir);                             % last: gisette helpers win
...
params.DEFLAT_LG_EIG       = 100;   % deflate the 40 largest eigenvectors of H
...
% Near-dense H needs much larger diagcomp shifts than the build_ichol_robust
% defaults before ict finds positive pivots.
...
rng(params.Seed);
dataFile = fullfile(thisFileDir, 'data', 'gisette_scale');
if ~exist(dataFile, 'file')
    error('run_benchmark:noData', ...
          'Missing %s -- run download_gisette() first.', dataFile);
end
...
Xa_sp = [Xs, sparse(ones(n, 1))];       % n-by-m, for explicit H
fprintf('[gisette] n=%d  d=%d  system size m=%d\n', n, dEff, m);
...
for j = 1:nL
    ...
    while it < params.NewtonMaxIter
        ...
        % --- exact reference step (advances the trajectory) ---
        % Direct Cholesky solve on the explicit SPD Hessian. The full-rank
        % gisette data term (n > m) makes the m-by-m factorization the sensible
        % reference; H_sp is already formed above for ichol/AMG.
        tRef  = tic;
        delta = H_sp \ rhs;
    ...
    title(sprintf('gisette logistic Newton PCG: lambda=%.0e (n=%d, d=%d, %d largest eigvecs)', ...
...
cfg_out.notes = {'All five solvers solve the identical (H_k, -g_k) per Newton step; trajectory advanced only by the exact reference solve delta = H_k \ (-g_k).', ...
                 ...
                 'H_sp is near-dense (dense standardized gisette X) -- ichol/AMG build cost caveat.', ...
...
fprintf('\n[gisette run_benchmark] done. Output in %s\n', outDir);
